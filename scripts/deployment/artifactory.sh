...
# -a [required] artifactory root location where R packages are stored along with latest.txt file pointing to the most recent package version
# -u [optional] user name to be authenticated against artifactory, set to currently logged in system user if not provided
...
if [ -z "${artifactory_root}" ]; then
    echo -a required input parameter value with artifactory root location was not provided!
    exit 1
fi
